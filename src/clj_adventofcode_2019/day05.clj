(ns clj-adventofcode-2019.day05
  (:require
...
(def data-file (io/resource "day05_input.txt"))
...
(defn execute-program
  ([program input output] (execute-program program 0 input output))
  ([program position input output]
   (let [opcode (nth program position)
     ...
     ;(println "pos:" position "op:" operation)
     (case operation
       (:add :mul) (let [first-param (get-param 1 opcode program position)
                     ...
                     ;(println " +/* -> " first-param second-param result destination-addr)
       ...
       :out (let [first-param (nth program (inc position))]
              ;(println " out[pos] " [first-param] (nth program first-param))
              (recur program next-position input (conj output (nth program first-param))
                     ))
       :exit [program input output]
       )

     )))

(def result-of-part1 (execute-program the-initial-program [1] []))
...
(defn execute-program-v2
  ([program input output] (execute-program-v2 program 0 input output))
  ([program position input output]
   (let [opcode (nth program position)
         operation (opcode-to-operation opcode)
         num-of-params (number-of-params operation)
...
(def part2-result (execute-program-v2 the-initial-program [5] []))
